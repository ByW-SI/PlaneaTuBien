...
<div class="card">
	...
	<form action="{{ route('prospectos.pagos.store', ['prospecto' => $prospecto]) }}" method="post" id="form">
		{{ csrf_field() }}
		<input type="hidden" name="prestamo_id" value="{{ $pago->prestamo->id }}">
		<input type="hidden" name="meses" value="{{ $pago->prestamo->meses }}">
				...
				<div class="col-sm-12">
					<div class="table-responsive">
						<table class="table table-sm table-bordered table-stripped table-hover">
							...
							<tr>
								<td>{{ $pago->prestamo->prestamo }}</td>
								<td>{{ $pago->created_at }}</td>
								<td>{{ $pago->prestamo->meses }}</td>
