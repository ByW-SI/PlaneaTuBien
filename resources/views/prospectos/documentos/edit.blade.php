...
<div class="card">
    ...
    <div class="card-body">
        ...
        <div class="card">
            ...
            @php($documento = $prospecto->documentos)
            <form action="{{ route('prospectos.documentos.update', ['prospecto' => $prospecto, 'documento' => $documento]) }}" method="post">
                {{ csrf_field() }}
                @method('PUT')
                <div class="card-body">
                    <div class="row">
                        <div class="form-group col-sm-3">
                            ...
                            <input type="date" name="nacimiento" class="form-control" required="" value="{{ $documento->nacimiento }}">
                        </div>
                        <div class="form-group col-sm-3">
                            <label class="control-label">✱Nacionalidad:</label>
                            <input type="text" name="nacionalidad" class="form-control" required="" value="{{ $documento->nacionalidad }}">
                        </div>
                        <div class="form-group col-sm-3">
                            <label class="control-label">Lugar de nacimiento:</label>
                            <input type="text" name="lugar" class="form-control" value="{{ $documento->lugar }}">
                        </div>
                        <div class="form-group col-sm-3">
                            <label class="control-label">✱Estado civil:</label>
                            <select name="civil" class="form-control" required="">
                                <option value="">Seleccionar</option>
                                <option value="Soltero"{{ $documento->civil == 'Soltero' ? ' selected' : 'N/a' }}>Soltero</option>
                                <option value="Casado"{{ $documento->civil == 'Casado' ? ' selected' : 'N/a' }}>Casado</option>
                                <option value="Divorciado"{{ $documento->civil == 'Divorciado' ? ' selected' : 'N/a' }}>Divorciado</option>
                                <option value="Viudo"{{ $documento->civil == 'Viudo' ? ' selected' : 'N/a' }}>Viudo</option>
                            </select>
                        </div>
                    </div>
                    <div class="row">
                        <div class="form-group col-sm-3">
                            ...
                            <input type="text" name="profesion" class="form-control" required="" value="{{ $documento->profesion }}">
                        </div>
                        <div class="form-group col-sm-3">
                            <label class="control-label">✱Empresa:</label>
                            <input type="text" name="empresa" class="form-control" required="" value="{{ $documento->empresa }}">
                        </div>
                        <div class="form-group col-sm-3">
                            <label class="control-label">✱Puesto actual:</label>
                            <input type="text" name="actual" class="form-control" required="" value="{{ $documento->actual }}">
                        </div>
                        <div class="form-group col-sm-3">
                            <label class="control-label">Puesto anterior:</label>
                            <input type="text" name="anterior" class="form-control" value="{{ $documento->anterior }}">
                        </div>
                    </div>
                    <div class="row">
                        <div class="form-group col-sm-3">
                            <label class="control-label">Antigüedad:</label>
                            <input type="text" name="antiguo" class="form-control" value="{{ $documento->antiguo }}">
                        </div>
                    </div>
                </div>
